...
impl Client {
    ...
    pub async fn log_in(&mut self, user: &str, password: &str) -> crate::Result<()> {
        ...
        login.set_params(RpcValue::new("magic"));
        let login = Frame::from_rpcmessage(Protocol::ChainPack, &login);
        self.connection.write_frame(&login).await?;
        let login_result = self.connection.read_frame().await?.to_rpcmesage()?;
