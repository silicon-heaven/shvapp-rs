...
use crate::{Request, Connection, Db, Frame, Shutdown};
...
impl Subscribe {
    ...
    /// to the server.
    pub(crate) fn into_frame(self) -> RpcMessage {
        let mut frame = RpcMessage::array();
...
fn make_subscribe_frame(channel_name: String, num_subs: usize) -> RpcMessage {
    let mut response = RpcMessage::array();
...
fn make_unsubscribe_frame(channel_name: String, num_subs: usize) -> RpcMessage {
    let mut response = RpcMessage::array();
...
fn make_message_frame(channel_name: String, msg: Bytes) -> RpcMessage {
    let mut response = RpcMessage::array();
