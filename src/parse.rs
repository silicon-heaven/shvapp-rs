...
impl Parse {
    ...
    pub(crate) fn new(frame: RpcMessage) -> Result<Parse, ParseError> {
        let array = match frame {
            RpcMessage::Array(array) => array,
    ...
    /// entry is a frame.
    fn next(&mut self) -> Result<RpcMessage, ParseError> {
    ...
    pub(crate) fn next_string(&mut self) -> Result<String, ParseError> {
        match self.next()? {
            ...
            // types.
            RpcMessage::Simple(s) => Ok(s),
            RpcMessage::Bulk(data) => str::from_utf8(&data[..])
                .map(|s| s.to_string())
    ...
    pub(crate) fn next_bytes(&mut self) -> Result<Bytes, ParseError> {
        match self.next()? {
            ...
            RpcMessage::Simple(s) => Ok(Bytes::from(s.into_bytes())),
            RpcMessage::Bulk(data) => Ok(data),
            frame => Err(format!(
                "protocol error; expected simple frame or bulk frame, got {:?}",
                frame
            )
    ...
    pub(crate) fn next_int(&mut self) -> Result<u64, ParseError> {
        ...
        match self.next()? {
            ...
            RpcMessage::Integer(v) => Ok(v),
            // Simple and bulk frames must be parsed as integers. If the parsing
            // fails, an error is returned.
            RpcMessage::Simple(data) => atoi::<u64>(data.as_bytes()).ok_or_else(|| MSG.into()),
            RpcMessage::Bulk(data) => atoi::<u64>(&data).ok_or_else(|| MSG.into()),
